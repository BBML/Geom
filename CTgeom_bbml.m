function [ ]  = CTgeom_bbml()
%% Revision History
% Edited Oct 10 2019 by Rachel Kohler to limit outputs (no longer gives
% c_ant, c_med, I_ap, and I_ml), and clean up code (deleting unused code).
...
try % use a try/catch block to run the code and save anything that has already ran
    ...
    geom_out_cell = cell(total_count, 14);
    ...
    for m=1:length(folders)
        ...
        for k=1:length(sub_folders)
            ...
            geom_out = zeros(length(slices)+2,14);
...
%                 Iml = Ivv * 1e-12;
                ...
                geometry = [total_cs_area marrow_area cortical_area cort_thickness ...
                    periosteal_BS endocortical_BS Imax ang_max Imin ang_min AP_width ML_width APtoMLratio tmd_HA];
                geom_out(j,:) = geometry;
            ...
            ang_max_rad = mean(geom_out(1:length(slices),7)).*pi./180;
            ang_min_rad = mean(geom_out(1:length(slices),9)).*pi./180;
            ...
            geom_out_cell(offset+k, 1:13) = mean_cell(1: 13);
            geom_out_cell(offset+k, 14) = num2cell(tmd_HA_avg);
    ...
    if exist('k', 'var') && exist('folder', 'var') && exist('j', 'var')
        ...
        if k+offset>1 % only attempt to save data if this is not the first sample analyzed
            ...
            headers = {'Total CSA (mm^2)', 'Marrow Area (mm^2)', 'Cortical Area(mm^2)', 'Cortical Thickness (mm)', 'Periosteal BS (mm)', 'Endocortical BS (mm)',  'Imax (mm^4)', 'Theta max (deg)', 'Imin (mm^4)','Theta min (deg)', 'AP Width (mm)', 'ML Width (mm)', 'AP/ML', 'TMD (g/cm^3 HA)'};
